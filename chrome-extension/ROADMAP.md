...
4. **Zero-dependency core.** The calculation engine stays pure, portable, and auditable. No npm install. No build step. The same `shared/contrast.js` that runs in the extension should run in the CLI, the Figma plugin, and the CI tool.
...
- **Design tool plugins (early).** Stark owns Figma. Our bridge to design tools comes through design token extraction and exportable reports, not native plugins.
